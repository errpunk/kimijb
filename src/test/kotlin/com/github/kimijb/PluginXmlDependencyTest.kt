...
import com.github.kimijb.action.KimiActionIds
...
class PluginXmlDependencyTest {
    ...
    @Test
    fun `plugin xml registers shortcut customization action`() {
        val stream = this::class.java.classLoader.getResourceAsStream("META-INF/plugin.xml")
        requireNotNull(stream) { "META-INF/plugin.xml not found in test resources" }
        val xml = stream.bufferedReader().use { it.readText() }

        assertTrue(
            xml.contains("""id="${KimiActionIds.CONFIGURE_SHORTCUT}""""),
            "plugin.xml must register a configure-shortcut action"
        )
        assertTrue(
            xml.contains("""group-id="ToolsMenu""""),
            "configure-shortcut action should be reachable from the Tools menu"
        )
    }
}
